...
function defineModel(name, attributes) {
    ...
    if(attrs.aId==undefined){ //防止文章id被重新定义
       attrs.id = {
            type: ID_TYPE,
            primaryKey: true
        }; 
        ...
        hooks: {
            ...
            beforeUpdate: function (obj) {
                ...
                if (obj.isNewRecord) {
                    ...
                    //console.log('1111will update entity...');
                    obj.updatedAt = now;
                    obj.version++;
                }
            }
...
const TYPES = ['STRING', 'CHAR', 'INTEGER', 'BIGINT', 'TEXT', 'DOUBLE', 'DATEONLY', 'BOOLEAN'];
...
var exp = {
    ...
    sync: () => {
        ...
        if (process.env.NODE_ENV !== 'production') {
            ...
            //类似这样：sequelize.sync({force: true});（加force:true，会先删掉表后再建表）
        } else {
